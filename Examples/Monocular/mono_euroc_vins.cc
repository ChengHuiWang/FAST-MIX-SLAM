...
void LoadImus(const string &strImuPath, vector<mix::IMUData> &vImus);
...
int main(int argc, char **argv)
{
    ...
    mix::System SLAM(argv[1],argv[2],mix::System::MONOCULAR,true);
    ...
    for(int ni=imagestart; ni<nImages; ni++)
    {
        ...
        vector<mix::IMUData> vimu;
        while(1)
        {
            const mix::IMUData& imudata = vImus[imuindex];
...
void LoadImus(const string &strImuPath, vector<mix::IMUData> &vImus)
{
    ...
    while(!fImus.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            data[0] *= 1e-9;
            mix::IMUData imudata(data[1],data[2],data[3],
